...
import djangoApi from '@/app/lib/djangoApiClient'; // Asegúrate que la ruta a tu cliente API sea correcta
...
export async function GET(request, { params }) {
  // Obtenemos el ID específico de la clase desde los parámetros de la URL.
  ...
  // Construimos la URL del endpoint de Django para una clase específica.
  // Asegúrate de que esta ruta exista en tu urls.py de Django.
  const djangoUrl = `/api/clases/clase/${id}/`;

  try {
    // 2. PRIMER INTENTO: Intentamos obtener el detalle de la clase.
    ...
    if (error.response?.status === 401) {
      console.log(`Access token expirado para la clase ${id}. Intentando refrescar...`);
      ...
      try {
        ...
        // 5. REINTENTO: Volvemos a hacer la petición original para el detalle de la clase.
    ...
    console.error(`Error al obtener el detalle de la clase ${id}:`, error.response?.data);
